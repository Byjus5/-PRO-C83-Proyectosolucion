...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      Alert.alert("Por favor, ingresa/escanea un id de biciclea válido");
      ...
      if (isEligible) {
        ...
        Alert.alert(
          "Has rentado la bicicleta por la siguiente hora. ¡Disfruta tu viaje!"
        );
        this.setState({
          bikeAssigned: true
        });
    ...
    if (bikeRef.docs.length == 0) {
      ...
      bikeRef.docs.map(doc => {
        if (!doc.data().under_maintenance) {
          //si la bicicleta está disponible, entonces el tipo de transacción será rentada
          // de lo contrario se devolverá 
  ...
  checkUserEligibilityForStartRide = async userId => {
    ...
    if (userRef.docs.length == 0) {
      ...
      Alert.alert("id de usario inválido");
    } else {
